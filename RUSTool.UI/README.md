...
界面皮肤全部来自项目内的设计系统（`Theme/`），业务能力来自 `RUSTool.Core`，
3D 视口来自 `RUSTool.Visualization`，曲线区来自 `RUSTool.Charts`（两条隔离带都只在本工程被引用一次）。
...
| 3D 视口的数据 / 线程契约 | [`../docs/visualization/zh-CN.md`](../docs/visualization/zh-CN.md) |
| 曲线区的数据 / 线程契约与配色 | [`../docs/charts/zh-CN.md`](../docs/charts/zh-CN.md) |
...
./preview.sh all        # 全部截图一次拍全（七张：四种皮肤组合 + 状态浮层 + 合成点云 + 合成力矩曲线）
./preview.sh torque     # 只拍「工程师模式 · 合成力矩曲线」那张（离线也能看见曲线）
```
