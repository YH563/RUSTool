...
public partial class Scene3DView : UserControl
{
    ...
    /// <summary>
    /// 视口顶部被覆盖层占掉的高度（逻辑像素）：工作区摆在 3D 卡片右上角的「机械臂状态」HUD。
    /// 只做一件事 —— 转发给 <see cref="RobotViewport.GizmoTopInset"/>。
    ///
    /// <para>
    /// 为什么要绕一道：HUD 是**工作区**那一层摆的（卡片内部只有视口，看不到它），
    /// 而右下角的朝向 gizmo 要避让的正是它。工作区把 HUD 的实测高度绑到这个属性上即可，
    /// 卡片内部不必知道 HUD 有多大、界面也不必知道 gizmo 怎么算。
    /// </para>
    /// </summary>
